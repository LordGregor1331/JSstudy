let value = prompt("Enter task name");
...
if (value === "number: odd") {
    ...
    let thirdquestion = confirm("Maybe coffee?") ? alert("How much sugar?") : "Maybe tea?";
} else if (value === "comparison sizes") {
...
} else if (value === "confirm on this days") {
    let shopping = confirm("Shopping?")
    shopping || alert("You are bad man")
    //OR
    let shopping1 = confirm("Shopping?")
    if (!shopping1) {
        alert("You are bad man")
    }
} else if (value === "default or") {
    ...
    alert(name + surname + middlename)
} else if (value === "login and password") {
...
} else if (value === "scissors paper rock") {
